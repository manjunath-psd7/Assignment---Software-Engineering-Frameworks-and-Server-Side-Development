...
public class UserController {
    ...
    public String registerUser(User user,Model model) {
        String returnUrl = "";
        if(evaluatePassword(user.getPassword()))
        {
    ...
    {
        final Pattern[] pwdPattern = new Pattern[3];
        pwdPattern[0] = Pattern.compile(".*[a-zA-Z].*");
        pwdPattern[1] = Pattern.compile(".*\\d.*");
        pwdPattern[2] = Pattern.compile(".*[`~ !@#$%^&*()\\-_=+\\\\|\\[{\\]};:'\",<.>/?].*");


        boolean isValid = true;
        for (Pattern pwd : pwdPattern) {
