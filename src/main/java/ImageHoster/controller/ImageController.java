...
public class ImageController {
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session) {
        ...
        List<Comment> comments = commentService.getComment(imageId);
        User user = (User) session.getAttribute("loggeduser");
        Boolean validateUser = compareUsers(user.getId(),image.getUser().getId());
        model.addAttribute("image", image);
        model.addAttribute("comments",comments);
        if(validateUser)
        {
            ...
            model.addAttribute("tags", image.getTags());
            returnUrl = "images/image";
        }
        else
        {
            String tags = convertTagsToString(image.getTags());
            model.addAttribute("tags", tags);
            returnUrl = "images/edit";
        }
        return returnUrl;
    }
    ...
    public String updateComment(@PathVariable("id") Integer id,@RequestParam("comment") String text,Comment comment,HttpSession session,Model model) {
        Image image = imageService.getImage(id);
        User user = (User)session.getAttribute("loggeduser");
    ...
    private Boolean compareUsers(Integer loggedInId,Integer imageUploaderId)
    {
        if(loggedInId != imageUploaderId)
            return true;
        else
            return false;
    }
